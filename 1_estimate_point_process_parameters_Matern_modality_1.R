...
# 
# 1_estimate_point_process_parameters_Matern_modality_1.R
# 
# jan.perret@gmail.com
#######################################################

###
### script objective : estimate the Matern process parameters to get the wanted levels of dispersion index - for modality 1 (intensity increased by increasing both kappa and mu)
###
...
wanted_disp_index = c(2:75) # vector with the wanted levels of dispersion index
mu_kappa_ratio = 10000 # for intensity = 1 it corresponds to : kappa = 0.01 ; mu = 100
...
# get the values of kappa and mu for each level of intensity given the value chosen for mu_kappa_ratio
kappa_vect = sqrt(wanted_intensities / mu_kappa_ratio)
mu_vect = mu_kappa_ratio * sqrt(wanted_intensities / mu_kappa_ratio)
intensity_vect = kappa_vect * mu_vect

# get the vector of initial values for scale parameter
scale_vect = c(0.001, seq(from = 0.05, to = 4.95, by = 0.05), seq(from = 5, to = 18, by = 0.5))
...
# but I choose scale = 20 as limit value because that's enough
# for the gradient of dispersion index of interest (there is no level
# of intensity for which the dispersion index exceeds 2 for scale > 20)
...
write.csv2(input_table, file = "./data/tidy/Matern_parameters_2021-06-13.csv", row.names = FALSE)



### refine the estimates
# load the table
Matern_parameters_tab <- read.csv2("./data/tidy/Matern_parameters_2021-06-13.csv")
...
# save results
write.csv2(parameter_tab, file = "./data/tidy/Matern_parameters_2021-06-13_refined.csv", row.names = FALSE)
write.csv2(convergence_fails, file = "./data/tidy/Matern_parameters_2021-06-13_convergence_fails.csv", row.names = FALSE)


### create the final dataframe with the parameter values (coef_tab_total)

wanted_intensities = c(1:30)
wanted_disp_index = c(2:75)

# load the table 
Matern_parameters_tab <- read.csv2("./data/tidy/Matern_parameters_2021-06-13_refined.csv")
...
# save data
write.csv2(coef_tab_total, file = "./data/tidy/Matern_parameters_coef_tab_total_2021-06-13.csv", row.names = FALSE)
...
#   ggtitle('Dispersion index as a function of the "scale" parameter of the Matern process (ratio mu / kappa fixed at 10000)') +
